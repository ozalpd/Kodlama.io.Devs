    ...
    where TContext : DbContext
{
    ...
    public async Task<IPaginate<TEntity>> GetListAsync(Expression<Func<TEntity, bool>>? predicate = null,
                                                       ...
                                                       Func<IQueryable<TEntity>, IIncludableQueryable<TEntity, object>>?
                                                           include = null,
                                                       int index = 0, int size = _defaultPageSize, bool enableTracking = true,
                                                       CancellationToken cancellationToken = default)
    {
        if (size < 1) size = _defaultPageSize;
        if (index < 0) index = 0;
    ...
    public async Task<IPaginate<TEntity>> GetListByDynamicAsync(Dynamic.Dynamic dynamic,
                                                                ...
                                                                int index = 0, int size = _defaultPageSize,
    ...
    public IPaginate<TEntity> GetListByDynamic(Dynamic.Dynamic dynamic, Func<IQueryable<TEntity>,
                                               IIncludableQueryable<TEntity, object>>? include = null,
                                               int index = 0, int size = _defaultPageSize,
                                               bool enableTracking = true)
    {
